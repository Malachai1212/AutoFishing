...
class SettingsWindow(QtWidgets.QMainWindow):
    ...
    potionTimer: int = 300
    baitType: str = "none"
    useFleetRepair: bool = False
    fleetRepairTimer: int = 600
    ...
    def __init__(self, parent: QtWidgets.QMainWindow):
        ...
        label_mealKey = Label(self, 0, 0, 0, 0, "label", "Lure key")
        label_mealKey.setAlignment(QtCore.Qt.AlignmentFlag.AlignLeft)
        self.__button_useMeal = Button(self, "Use lure", 0, 0, 0, 0, "btn_red", self.changeMealFlag)
        self.__entry_mealKey = Entry(self, 0, 0, 0, 0, "EMPTY", False, "entry_standart")
        label_mealTimer = Label(self, 0, 0, 0, 0, "label", "Lure timer")
        ...
        label_seaChoise = Label(self, 0, 0, 0, 0, "label", "Sea choice")
        ...
        label_baitType = Label(self, 0, 0, 0, 0, "label", "Bait type")
        label_baitType.setAlignment(QtCore.Qt.AlignmentFlag.AlignLeft)
        self.__bait_combo = QtWidgets.QComboBox(self)
        self.__bait_combo.addItems(["None", "Normal Bait", "Swarm Bait", "Giant Bait", "Magic Bait"])
        grid.addWidget(label_baitType, 6, 0)
        grid.addWidget(self.__bait_combo, 6, 1, 1, 2)

        label_fleetRepair = Label(self, 0, 0, 0, 0, "label", "Fleet repair")
        label_fleetRepair.setAlignment(QtCore.Qt.AlignmentFlag.AlignLeft)
        self.__button_useFleetRepair = Button(self, "Repair fleet", 0, 0, 0, 0, "btn_red", self.changeFleetRepairFlag)
        label_fleetRepairTimer = Label(self, 0, 0, 0, 0, "label", "Repair timer")
        label_fleetRepairTimer.setAlignment(QtCore.Qt.AlignmentFlag.AlignLeft)
        self.__entry_fleetRepairTimer = Entry(self, 0, 0, 0, 0, "EMPTY", False, "entry_standart")
        grid.addWidget(label_fleetRepair, 7, 0)
        grid.addWidget(self.__button_useFleetRepair, 7, 1, 1, 2)
        grid.addWidget(label_fleetRepairTimer, 8, 0)
        grid.addWidget(self.__entry_fleetRepairTimer, 8, 1, 1, 2)
    ...
    def changeFleetRepairFlag(self) -> None:
        self.useFleetRepair = not self.useFleetRepair
        if self.useFleetRepair: self.__button_useFleetRepair.setObjectName("btn_standart")
        else: self.__button_useFleetRepair.setObjectName("btn_red")
        self.setStyleSheet(CSS)
    ...
    def clearEntrys(self) -> None:
        ...
        self.__entry_fleetRepairTimer.clear()
        ...
        self.__entry_fleetRepairTimer.setObjectName("entry_standart")
        self.setStyleSheet(CSS)

        if self.timeForTry == 22.5: self.__sea_combo.setCurrentIndex(0)
        elif self.timeForTry == 30: self.__sea_combo.setCurrentIndex(1)

        bait_map = {"none": 0, "normal": 1, "swarm": 2, "giant": 3, "magic": 4}
        self.__bait_combo.setCurrentIndex(bait_map.get(self.baitType, 0))
    ...
    def saveChanges(self) -> None:
        ...
        check6 = self.checkEntryWithTime(self.fleetRepairTimer, self.__entry_fleetRepairTimer)
        ...
        self.fleetRepairTimer = check6[0]
        sea = self.__sea_combo.currentText()
        if sea == "Normal sea": self.timeForTry = 22.5
        elif sea == "Dark sea": self.timeForTry = 30

        bait = self.__bait_combo.currentText()
        bait_reverse = {"None": "none", "Normal Bait": "normal", "Swarm Bait": "swarm", "Giant Bait": "giant", "Magic Bait": "magic"}
        self.baitType = bait_reverse.get(bait, "none")
        ...
        if check1[1] and check2[1] and check3[1] and check4[1] and check5[1] and check6[1]:
            ...
            newDBobject = {
                "settings": [{
                    ...
                    "useFleetRepair": self.useFleetRepair,
                    "fleetRepairTimer": self.fleetRepairTimer
                }],
    ...
    def readDataBase(self) -> None:
        ...
        self.baitType = settings.get("baitType", "none")
        self.useFleetRepair = settings.get("useFleetRepair", False)
        self.fleetRepairTimer = settings.get("fleetRepairTimer", 600)
        ...
        if self.useFleetRepair: self.__button_useFleetRepair.setObjectName("btn_standart")
        self.__entry_fleetRepairTimer.setPlaceholderText(f"{self.fleetRepairTimer} seconds")
